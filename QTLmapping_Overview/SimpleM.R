###
##See: https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2796708/

setwd("C:/Users/Manuel/Desktop/AIL_B6xBFMI/RAWDATA")
numgeno <- read.csv("genomatrix.clean_numeric.txt", header = TRUE, check.names = FALSE, sep="\t", colClasses="character")
...
inferCutoff <- function(dt_My, PCA_cutoff = 0.995){ # infer the cutoff => Meff
  #dt_My <- dt_My[, ! apply(dt_My , 2 , function(x) sd(x, na.rm = TRUE)==0 )]
  CLD <- cor(dt_My, use = "pairwise.complete.obs")
  eigen_My <- eigen(CLD)
...
for(x in rangeTested){
  ...
  while (myStop < numLoci) {
	...
	snpInBlk <- t(apply(snpInBlk,1,function(x){as.numeric(x)}))
